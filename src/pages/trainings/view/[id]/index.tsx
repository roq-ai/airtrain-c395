...
import { deleteEmployeeTrainingById } from 'apiSdk/employee-trainings';
...
function TrainingViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TrainingInterface>(
    ...
    () =>
      getTrainingById(id, {
        relations: ['company', 'training_template', 'employee_training'],
      }),
  );

  const employee_trainingHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
            ...
            <Box>
              ...
              {hasAccess('training_template', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/training-templates/view/${data?.training_template?.id}`}>
                      {data?.training_template?.name}
                    </Link>
                  </Text>
                </Flex>
              )}
              {hasAccess('employee_training', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/employee-trainings/create?training_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.employee_training?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/employee-trainings/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess(
                                ...
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/employee-trainings/edit/${record.id}`}
                                >
                              ...
                              {hasAccess(
                                ...
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    employee_trainingHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
