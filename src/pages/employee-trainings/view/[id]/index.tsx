...
import { getEmployeeTrainingById } from 'apiSdk/employee-trainings';
...
function EmployeeTrainingViewPage() {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Employee Training Detail View
          </Text>
          {hasAccess('employee_training', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/employee-trainings/edit/${data?.id}`} passHref legacyBehavior>
              <Button
            ...
            <Box>
              ...
              {hasAccess('training', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/trainings/view/${data?.training?.id}`}>
                      {data?.training?.name}
                    </Link>
                  </Text>
                </Flex>
